import Button from "@/app/ui/button";
import images from "@/public/images";
import Image from "next/image";

export default function HeroBanner() {
    return (
    <div className={`md:px-[8%] relative mt-12 md:mt-20 `}>
      <div className="md:h-[264px] h-fit w-full md:py-0 py-8 bg-gradient-to-r from-[#FFD700] via-[#0F2B22] to-[#FFD700] flex items-center md: gap-12 md:px-16 px-4 md:rounded-3xl ">
        <div className=" md:block hidden -translate-y-[48px]">
            <Image src={images.man} alt="man" />
        </div>
        <div className="flex w-full md:w-1/2 flex-col gap-2">
          <h3 className="text-white md:px-0 px-8 md:text-start text-center ">
            Have a Question? We're Here to Help!
          </h3>
          <p className="text-[#DBDADA] md:mt-0 mt-2 md:text-start text-center ">
          Got inquiries about our products, orders, or anything else? Reach out to our team and we'll assist you with the right answers. Whether you're shopping for men's fashion, women's wear, or kids' gadgets, we're always ready to help you make the best choice.
          </p>
         <Button>
            Make Enquiry
         </Button>
        </div>
      </div>
    </div>
    )
}